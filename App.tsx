...
export default function App() {
  ...
  const handleSend = async (textOverride?: string, options: { isLive?: boolean } = {}) => {
       ...
       if (options.isLive && isLiveModeRef.current) {
          try {
             ...
             if (audioData) {
               ...
               // Fallback if audioData is null
               speakWithBrowser(identityOverride);
             }
          } catch (e) { 
             console.warn("Identity TTS fail, using fallback", e);
             speakWithBrowser(identityOverride);
          }
       }
       return; 
    ...
    try {
      ...
      if (isImageGen) {
        ...
        if (options.isLive && isLiveModeRef.current) {
          try { 
             ...
             // We catch here but audioData stays null
          }
        }
      }
      ...
      responseText = "I'm having trouble connecting right now. Please check your internet or API key.";
      if (!options.isLive) setMessages(prev => [...prev, { id: Date.now().toString(), role: 'model', text: responseText, timestamp: Date.now() }]);
      
      // If Live Mode and Generation Error, Speak the Error
      if (options.isLive && isLiveModeRef.current) {
         speakWithBrowser(responseText);
         return; // Exit after handling error speech
      }
    } finally {
      setLoading(false);
    }

    // Success Path for UI & Audio
    if (!generationError) {
      ...
      if (options.isLive && isLiveModeRef.current) {
         if (audioData) {
            ...
            // Fallback to Browser Voice if Gemini Audio failed but Text worked
            speakWithBrowser(responseText);
         }
      }
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'} flex h-[100dvh] font-sans overflow-hidden relative transition-colors duration-300`}>
      ...
      <div className={`fixed z-50 md:relative h-full w-[85%] sm:w-72 flex flex-col transition-transform duration-300 ease-in-out border-r shadow-2xl md:shadow-none ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'} ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className={`p-4 border-t space-y-2 ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}>
      ...
      <div className="flex-1 flex flex-col h-full relative w-full">
        ...
        {/* MESSAGES LIST */}
        <div className={`flex-1 overflow-y-auto p-3 md:p-6 space-y-6 md:space-y-8 scroll-smooth ${isDarkMode ? 'bg-gray-900' : 'bg-[#f0f2f5]'}`}>
          {messages.length === 0 && (
        ...
        {/* INPUT AREA */}
        <div className={`shrink-0 p-2 md:p-4 border-t z-20 ${isDarkMode ? 'border-gray-800 bg-gray-900' : 'border-gray-200 bg-white'}`}>
           {attachment && (
           ...
           <div className={`flex items-end gap-2 md:gap-3`}>
              {/* TOOLS BAR (Mobile: Integrated, Desktop: Integrated) */}
